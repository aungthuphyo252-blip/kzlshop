...
const AuthWrapper = ({ hasUsers, hasAdmin, onCreateUser, onLogin, onGoogleLogin, townships }) => {
  ...
  useEffect(() => {
    ...
    else if (!hasAdmin) setMode('create-admin');
    else setMode('login');
  }, [hasUsers, hasAdmin]);
    ...
    return (
      <div style={{ padding: 24, display: 'flex', gap: 20, justifyContent: 'center' }}>
        <SignupForm
          ...
          submitLabel={hasUsers ? 'Create Admin Account' : 'Create admin account'}
          townships={townships}
        />
      </div>
    );
  ...
  return (
    <div style={{ padding: 24, display: 'flex', gap: 20, flexWrap: 'wrap', justifyContent: 'center' }}>
      {mode === 'login'
        ...
        : <SignupForm onSubmit={handleSignup} switchToLogin={() => setMode('login')} townships={townships} />}
    </div>
  );
};
...
const ProfileView = ({ user, updateUser, userOrders, deleteOrder, onBack, isViewingOther, townships = [] }) => {
  ...
  const handleDeleteOrder = async (orderId) => {
    ...
    if (res.success) alert('Order deleted'); else alert('Delete failed');
  };
          ...
          <>
            ...
            <div className="table-wrap">
              <table className="orders-table">
                <thead>
                  <tr><th>ID</th><th>User</th><th>Product</th><th>Qty</th><th>Total</th><th>Status</th><th>Action</th></tr>
                </thead>
                <tbody>
                  {(orders || []).map(o => (
                    <tr key={o.id}>
            ...
            <form style={{ display: 'grid', gap: 12, maxWidth: 720 }} onSubmit={saveSettings}>
              ...
              <div>
                ...
                <div className="upload-row">
                  ...
                  <div className="upload-help muted">Upload a logo image (saved as data URL in demo).</div>
                </div>
              ...
              <div>
                ...
                <div className="upload-row">
                  ...
                  <div className="upload-help muted">Optional splash image for app intro (saved as data URL in demo).</div>
                </div>
...
/* -------------------------
   AppMenu (inline)
   ------------------------- */
function AppMenu({ isAdmin, hasAdmin, unseenCount = 0, setView = () => {}, onLogout = () => {}, openCreateAdminPrompt = () => {}, setSelectedProfileUserId = () => {} }) {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      }, (err) => { console.error(err); setUsers([]); setHasUsers(false); setHasAdmin(false); setLoadingError(`Users listener: ${err?.message}`); });
  ...
  const handlePlaceOrder = async (orderData) => {
    ...
    if (!res.success) { alert('Order failed'); return; }
    ...
    return (
      <div style={{ minHeight: '100vh' }}>
            ...
            <div className="brand-center">
              <div className="brand-block">
                <div className="brand-logo">
                  {appSettings.logoUrl ? <img src={appSettings.logoUrl} alt="logo" style={{ width: 48, height: 48, objectFit: 'cover', borderRadius: 10 }} /> : 'KZ'}
                </div>
                <div>
        ...
        <main className="main container">
          <AuthWrapper hasUsers={hasUsers} hasAdmin={hasAdmin} onCreateUser={handleCreateUser} onLogin={handleLogin} onGoogleLogin={handleGoogleLogin} townships={townships} />
        </main>
  ...
  const confirmQtyAddToCart = () => {
    ...
    // do not open cart automatically; show feedback only
    setShowQtyPrompt(false);
    setSelectedProduct(null);
    setOrderQuantity(1);
  };
  ...
  const confirmCartToOrders = async () => {
    ...
    try {
      ...
      alert('Your order(s) have been placed. Admin will confirm (Order Received) later.');
    } catch (e) {
  ...
  return (
    <div className="app-root">
      ...
      {showOrderModal && selectedProduct && (
        <div className="modal-backdrop">
          <div className="modal">
            <button className="btn-close" onClick={() => { setShowOrderModal(false); setSelectedProduct(null); setOrderQuantity(1); }}>×</button>
            <h2>Confirm Order</h2>
